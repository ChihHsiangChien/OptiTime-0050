    ...
    if not st.session_state.game_active:
        ...
        * **交易摩擦成本**：買入需支付 **0.1425%** 手續費；賣出需支付 **0.1425%** 手續費 ＋ **0.3%** 證券交易稅（共 **0.4425%**）。
        ...
        if setup_idx < total_setups:
            ...
            * **交易手續費與稅金**：您每次進行買賣，都需要付給政府與券商約 0.58% 的摩擦成本。隨著交易次數增加，這些隱形成本會迅速蠶食您的本金。
            * **為什麼長期持有（存股）是贏家？**：如上面的累積績效對比所示，即使市場有波動，**什麼都不做、單純持有的「存股」策略**，由於沒有頻繁進出產生的手續費，且完整參與了市場的上漲，最終往往能擊敗大多數試圖預測高低點的交易者。
            * **💡 跨回合持有的交易成本優勢（您的持股 vs 系統 B&H）**：
              如果您選擇「跨回合持有股票且不賣出」，您會省下中途回合切換時的交易稅費（因為您的股票沒有被實質賣出再買回，系統已透過股數平抑為您保留了資產價值）。
              相反地，系統數據中的「長期持有 B&H」對照組，由於是在多個隨機不連續時段運作，每回合的起點與終點都必須強制重新買入與賣出（因而每次都多扣了雙邊手續費與稅金）。
              **這也是為什麼若您也是第一天買進後一路抱到最後，您的累積報酬會稍微高於對照組的 B&H**，這真實反映了「長期持有能省下交易摩擦成本」的金融特性！
            """)
